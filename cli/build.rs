...
//
// We intentionally do NOT invoke `boot/build.sh` from here, even though
// that would make `cargo build --release` a single self-contained step.
// Reason: `boot/build.sh` itself runs `cargo build -p zboot-boot --release`
// (the initramfs's PID 1, a separate binary in the same workspace),
// which would deadlock on cargo's workspace build lock when this build
// script is itself running inside `cargo build`.  The two-step manual
// flow (`bash boot/build.sh && cargo build --release -p zboot-cli`) is
// the only race-free option without restructuring zboot-boot out of
// the workspace.
